...
const ProfileListItems = () => {
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      const response = await apiEndpoints.getUserOrders()
      ...
      if (response.data.success && response.data.orders) {
        ...
        const processedOrders = response.data.orders.filter(order =>
          order.items &&
        ...
        if (processedOrders.length > 0) {
          ...
          setOrderData(processedOrders.reverse().slice(0, 6))

          // Buy again (shuffled from all orders, max 5)
          setOrderDataOne(shuffleArray([...processedOrders]).slice(0, 5))
        } else {
        ...
        console.log('API returned success: false or no orders')
        setOrderData([])
        setOrderDataOne([])
      }
    } catch (error) {
