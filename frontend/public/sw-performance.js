...
const CACHE_NAME = 'inkdapper-performance-v1';
const STATIC_CACHE = 'inkdapper-static-v1';
const DYNAMIC_CACHE = 'inkdapper-dynamic-v1';
const API_CACHE = 'inkdapper-api-v1';
  ...
  event.waitUntil(
    Promise.all([
      ...
      caches.open(API_CACHE).then((cache) => {
        console.log('Performance SW: Caching API endpoints');
        return Promise.all(
          API_ENDPOINTS.map(endpoint => 
            fetch(endpoint)
              ...
              .catch(error => {
                console.log('Performance SW: Failed to cache', endpoint, error);
              })
          )
        );
      })
      ...
      caches.open(API_CACHE).then((cache) => {
        return cache.match(request).then((cachedResponse) => {
          ...
          return fetch(request).then((response) => {
            ...
            // Network failed, return offline response for critical APIs
